<?php 
    include 'includes/global-header.php';
?>

<main>
    <div class="cross-shapes contact-shapes">
        <img src="assets/icons/logo-shape-small.svg" class="floating-img logo-shape-small" alt="">
        <img src="assets/icons/logo-shape.svg" class="floating-img logo-shape" alt="">
        <img src="assets/icons/logo-shape-extra-small.svg" class="floating-img logo-shape-extra-small" alt="">
    </div>
    <section class="page-padding single-page">
        <div class="section-container">
            <h1 class="section-title-main">Kontakt</h1>
                <p class="section-subtitle">Slobodno nas kontaktirajte ako imate bilo kakva pitanja.</p>
        </div>
    </section>
    <section>
        <div class="section-container">
            <div class="static-tabs">
                <a href="https://vivex.rs/rs/kontakt" class="static-tab">Srbija</a>
                <div class="static-tab active">Bosna i Hercegovina</div>
            </div>
        </div>
    </section>
    <section>
        <div class="section-container">
        <div class="contact-block">
                    <div class="contact-block-left">
                        <h2 class="contact-block-title">Prodaja i konsultacije</h2>
                        <div class="contact-block-item">
                            <div class="contact-block-icon">
                                <i class="fi fi-rs-mobile-button"></i>
                            </div>
                            <div class="contact-block-data">
                                <span>Mobilni:</span>
                                <a href="tel:+381 63 685 015">+381 63 685 015</a>
                            </div>
                        </div>
                        <div class="contact-block-item">
                            <div class="contact-block-icon">
                                <i class="fi fi-rs-phone-call"></i>
                            </div>
                            <div class="contact-block-data">
                                <span>Telefon:</span>
                                <a href="tel:+381 11 317 02 99">+381 11 317 02 99</a>
                            </div>
                        </div>
                        <div class="contact-block-item">
                            <div class="contact-block-icon">
                                <i class="fi fi-rs-phone-call"></i>
                            </div>
                            <div class="contact-block-data">
                                <span>Telefon:</span>
                                <a href="tel:+381 11 317 03 09">+381 11 317 03 09</a>
                            </div>
                        </div>
                        <div class="contact-block-item">
                            <div class="contact-block-icon">
                                <i class="fi fi-rs-envelope"></i>
                            </div>
                            <div class="contact-block-data">
                                <span>Email:</span>
                                <a href="#" class="modal-open">Send us an e-mail</a>
                            </div>
                        </div>
                        <div class="contact-block-item">
                            <div class="contact-block-icon">
                                <i class="fi fi-rs-marker"></i>
                            </div>
                            <div class="contact-block-data">
                                <span>Adresa:</span>
                                <a href="https://maps.app.goo.gl/AbkotR5auMU6iKzA7">Konstantina Jovanovića 41, 11080 Belgrade, Serbia</a>
                            </div>
                        </div>
                        <div class="contact-block-item">
                            <div class="contact-block-icon">
                                <i class="fi fi-rs-marker"></i>
                            </div>
                            <div class="contact-block-data">
                                <span>Radno vrijeme:</span>
                                <strong>Ponedeljak - petak / 08:00 - 16:00</strong>
                            </div>
                        </div>
                    </div>
                    <div class="contact-block-right">
                        <iframe src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d11314.390531570687!2d20.3422319!3d44.850127!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x475a666a318aa7fd%3A0x39768328592dde5d!2sVivex%20Group%20d.o.o.!5e0!3m2!1shr!2sba!4v1714318635841!5m2!1shr!2sba" width="100%" height="100%" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                    </div>
                </div>
        </div>
    </section>
    <section>
    <div class="section-container">
                <div class="contact-block">
                    <div class="contact-block-left">
                        <h2 class="contact-block-title">Servis</h2>
                        <div class="contact-block-item">
                            <div class="contact-block-icon">
                                <i class="fi fi-rs-mobile-button"></i>
                            </div>
                            <div class="contact-block-data">
                                <span>Mobilni:</span>
                                <a href="tel:+381 63 649 101">+381 63 649 101</a>
                            </div>
                        </div>
                        <div class="contact-block-item">
                            <div class="contact-block-icon">
                                <i class="fi fi-rs-phone-call"></i>
                            </div>
                            <div class="contact-block-data">
                                <span>Telefon:</span>
                                <a href="tel:+381 11 213 15 73">+381 11 213 15 73</a>
                            </div>
                        </div>
                        <div class="contact-block-item">
                            <div class="contact-block-icon">
                                <i class="fi fi-rs-phone-call"></i>
                            </div>
                            <div class="contact-block-data">
                                <span>Telefon:</span>
                                <a href="tel:+381 11 311 99 51">+381 11 311 99 51</a>
                            </div>
                        </div>
                        <div class="contact-block-item">
                            <div class="contact-block-icon">
                                <i class="fi fi-rs-envelope"></i>
                            </div>
                            <div class="contact-block-data">
                                <span>Email:</span>
                                <a href="#" class="modal-open">Send us an e-mail</a>
                            </div>
                        </div>
                        <div class="contact-block-item">
                            <div class="contact-block-icon">
                                <i class="fi fi-rs-marker"></i>
                            </div>
                            <div class="contact-block-data">
                                <span>Adresa:</span>
                                <a href="https://maps.app.goo.gl/YP9UUXz7ruXHPBn6A">Antifašističke borbe 27, 11070 Belgrade, Serbia</a>
                            </div>
                        </div>
                        <div class="contact-block-item">
                            <div class="contact-block-icon">
                                <i class="fi fi-rs-marker"></i>
                            </div>
                            <div class="contact-block-data">
                                <span>Radno vrijeme:</span>
                                <strong>Ponedeljak - petak / 08:00 - 16:00</strong>
                            </div>
                        </div>
                    </div>
                    <div class="contact-block-right">
                    <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2830.4435939095965!2d20.422146276238685!3d44.81252687107086!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x475a65607b2c8e93%3A0xcf0d9a48c2d6ca06!2sVivex%20Group%20d.%20o.%20o.%20PJ%20Servis!5e0!3m2!1shr!2sba!4v1714320797380!5m2!1shr!2sba" width="100%" height="100%" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                    </div>
                </div>
        </div>
    </section>
</main>
<?php 
    include 'includes/global-footer.php'; 
?>